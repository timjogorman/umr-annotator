...
Options:
  ...
  --mrp                     use MRP codec (reading MRP json) instead of generic PENMAN one
'''
...
def node_order(triples):
    """
    Return a list of triples in the original order.
    """
    nod = {}
    for t in triples:
        if (isinstance(t.target, str) and  t.target.replace("x","").isdigit()):
            nod[t] = (int(t.source.replace("x","")), int(t.target.replace("x","")))
        else:
            nod[t] = (int(t.source.replace("x","")), 0)

    return sorted(triples, key=lambda x:nod[x])

def exact_order(triples):
    inst = [x for x in triples if x.relation == 'instance']
    rel = [x for x in triples if x.relation != 'instance']
    actual_triples = [inst.pop(0)]
    seen_variable = actual_triples[0].source
    for var in range(len(inst)):

        unusedrel = []
        for r in rel:
            if r.source in seen_variable and (r.target in seen_variable or r.target == inst[0].source):
                actual_triples.append(r)
            else:
                unusedrel.append(r)
        rel = unusedrel[:]
        if len(inst) > 0:
            actual_triples.append(inst.pop())
        if len(inst) ==0:
            actual_triples += rel
            break

    return actual_triples
...
class PENMANCodec(object):
    ...
    def handle_triple(self, lhs, relation, rhs):
        ...
        if False and self.is_relation_inverted(relation):  # deinvert
...
class MRPCodec(PENMANCodec):
    ...
    def __init__(self, indent=True, relation_sort=node_order):
        """
        Initialize the MRP codec.
    ...
    def _namenode(self, varname):
        return "x"+str(varname)
    def iterdecode(self, s, triples=False):
        """
        Convert list of MRP JSONS into its Graph objects.

        Args:
            s: a string containing zero or more MRP Jsons
            triples: if True, treat *s* as a conjunction of logical triples
        Yields:
            valid Graph objects described by MRPs in *s*
        Example:

            >>> codec = PENMANCodec()
            #################################
            ######### EXAMPLE OUTDATED UPDATE ME
            ####################################
            >>> list(codec.iterdecode('(h / hello)(g / goodbye)'))
            [<Graph object (top=h) at ...>, <Graph object (top=g) at ...>]
            >>> list(codec.iterdecode(
        ...
        for line in s:
            #print(line)
            if not 'tops' in line:
                logging.error(f"no top in {line['id']} using first node!!")
                
                if len(line['nodes']) == 0:
                    continue
                line['tops'] =[line['nodes'][0]['id']]
            if len(line['tops']) > 1:
                logging.error(f"multiple tops in {line['id']}, adding artificial links")
                
                for other in line['tops'][1:]:
                    line['edges'].append({"source":line['tops'][0],"target":other,"label":"artificial-dag-link"})
                line['tops'] = [line['tops'][0]]
                
            assert(len(line['tops']) == 1)
            top = self._namenode(line['tops'][0])
            nodes = []
            anchors = {}
            for x in line['nodes']:

                if 'label' in x:
                    nodes.append((self._namenode(x['id']), 'instance', x['label']))
                else:
                    logging.error("unlabeled node! (this is an error for all but UCCA)")
                    nodes.append((self._namenode(x['id']), 'instance', "UNLABELED"))
                if 'anchors' in x:
                    sv=  []
                    for an in x['anchors']:
                        if 'from' in an:
                            sv = list(range(an['from'], an['to']))
                        else:

                            print(x)
                            print(sv)
                            input("###")
                    anchors[self._namenode(x['id'])] = anchors.get(self._namenode(x['id']),[]) + sv
            names = [(self._namenode(x['id'])) for x in line['nodes'] if x.get('label',[])=='name']
            edges = [(self._namenode(x['source']), x['label'], self._namenode(x['target'])) for x in line['edges']]

            attributes = []
            for node in line['nodes']:
                for attrid, rel in enumerate(node.get('properties', [])):

                    val = node['values'][attrid]
                    if self._namenode(node['id']) in names or not (val.isdigit() or val in ['imperative','expressive',"-","+"]):
                        val = '"'+val+'"'
                    attributes.append((self._namenode(node['id']), rel, val))
            nodevars = [x[0] for x in nodes]
            linkedto = [x[2] for x in edges] + [x[0] for x in edges if x[2] in nodevars]
            j = [n for n in nodevars if not (n in linkedto or n in top)]
            s = self.triples_to_graph(nodes + edges + attributes, top=top)
            try:
                str(s)
            except:
                isbad = True
                while isbad:
                    logging.error(f"disconnected graph in {line['id']}, adding artificial links")
                    dc= self._determine_triples_to_add_to_disconnected_graph(s, top)
                    corrections = {}
                    for q in dc:
                        corrections[q.source] = corrections.get(q.source, 0) + 1
                        corrections[q.target] = corrections.get(q.target, 0) -1
                    additions = [x for x in corrections if corrections[x] == max(corrections.values())]
                    for a in additions:
                        edges += [(self._namenode(line['tops'][0]),"artificial-dag-link", a)]
                    s = self.triples_to_graph(nodes + edges + attributes, top=top)
                    try:
                        str(s)
                        isbad = False
                        ...
                        pass            
            s._metadata['id'] = line['id']
            s._metadata['sentence'] = line['input']
            s._metadata['framework'] = line['framework']
            s._metadata['alignments'] = anchors
            yield s
            #print(nodes)
            #input("#")                    
        #input("ONOW")
    def encode(self, g, top=None, triples=False):
        ...
        if triples:
            ...
            rawstring = self._encode_penman(g, top=top)
            if 'alignments' in g._metadata:
                header1 = "\n".join(["# ::"+x+" "+g._metadata[x] for x in g._metadata if not x == 'alignments'])
                header2 = "# ::alignments "+" ".join([x+":"+",".join([str(char_index) for char_index in g._metadata['alignments'][x]]) for x in g._metadata['alignments']])
                header2b = "# ::alignments_text "+" ".join([x+":"+"".join([g._metadata['sentence'][char_index] for char_index in g._metadata['alignments'][x]]) for x in g._metadata['alignments']])
                return header1+'\n'+header2+'\n'+header2b+"\n"+rawstring
            else:
                return header1+"\n"+rawstring
            #print(header2)
...
class Graph(object):
    ...
    def anchored_triples(self, source=None, relation=None, target=None):
        ...
        l = list(filter(triplematch, self._triples))
        instance_anchors = []
        other_anchors = []
        for each_point in l:
            if each_point.relation == 'instance':
                rid =raw.find("("+each_point.source+" / ")
                instance_anchors.append([rid, each_point])
            else:
                other_anchors.append(each_point)
        a = sorted(instance_anchors)
        return [x[1] for x in a] + other_anchors
...
def loadjson(source, triples=False, cls=PENMANCodec, **kwargs):
    """
    Deserialize a list of PENMAN-encoded graphs from *source*.
    ...
    Returns:
        a list of Graph objects
    """
    decode = cls(relation_sort=original_order).iterdecode
    if hasattr(source, 'read'):
        return list(decode([json.loads(x) for x in source.readlines() if x.strip() != '']))
    else:
        with open(source) as fh:
            return list(decode([json.loads(x) for x in fh.readlines() if x.strip() != '']))
...
def _main():
    ...
    if args['--mrp']:
        codec = MRPCodec
        data = loadjson(infile, cls=codec)
    else:
